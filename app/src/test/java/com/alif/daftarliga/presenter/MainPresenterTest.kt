...
class MainPresenterTest {
    ...
    fun setUp() {
        ...
        mainPresenter = MainPresenter(
            mainView, apiRepository, gson, TestContextProvider()
        )
    }

    @Test
    fun testGetLeagueDataFromAPI() {
        /*val leagueIds: Array<String> =
            arrayOf("4328", "4334", "4331", "4332", "4335", "4346", "4344", "4356", "4330", "4396")*/
        val leagueIds: Array<String> = arrayOf("4328")

        val league = League(
            "4328", "1", null, "Soccer", "English Premier League",
            "Premier League", "0", "0", "1992", "2000-08-18",
            "Male", "England", "www.premierleague.com", "en-gb.facebook.com/premierleague/",
            "twitter.com/premierleague", "www.youtube.com/channel/UCG5qGWdu8nIRZqJ_GgDwQ-w",
        ...
        val nextEvent = Event(
            "602355", "402206", null, "Burnley vs Leicester",
            "Leicester @ Burnley", "English Premier League 2020-01-18 Burnley vs Leicester",
            "Soccer", "4328", "English Premier League", "1920", null,
            "Burnley", "Leicester", null, "23", null, null,
            "", "", "", "", "", "", "", "",
            null, "", "", "", "", "", "", "", "",
            null, null, null, "2020-01-19", null, "19/01/20",
            "14:00:00", "15:00:00", null, "133623", "133626",
            null, null, null, null, null, null, null, null,
            null, null, null, null, null, "unlocked"
        )

        val prevEvent = Event("602358","402209",null,"Man City vs Crystal Palace",
            "Crystal Palace @ Man City","English Premier League 2020-01-18 Man City vs Crystal Palace",
            "Soccer","4328","English Premier League","1920",null,
            "Man City","Crystal Palace","2","23","2",null,
            "82':Sergio Aguero;87':Sergio Aguero;","","58':Benjamin Mendy;",
            "Ederson Moraes; ","Kyle Walker; John Stones; Fernandinho; Benjamin Mendy; ",
            "Bernardo Silva; Rodri; Kevin De Bruyne; ","Riyad Mahrez; Sergio Aguero; Raheem Sterling; ",
            "Claudio Bravo; Kyle Walker; Nicolas Otamendi; Rodri; Gabriel Jesus; Riyad Mahrez; Phil Foden; ",
            null,"","13':James McArthur;26':Jordan Ayew;",
            "39':Cenk Tosun;73':Crystal Palace;90':Fernandinho (Own goal);","Vicente Guaita; ",
            "Martin Kelly; James Tomkins; Gary Cahill; Jairo Riedewald; ","Cheikhou Kouyate; James McArthur; James McCarthy; ",
            "Jordan Ayew; Wilfried Zaha; Connor Wickham; ","Wayne Hennessey; Joel Ward; Scott Dann; Sam Woods; Giovanni McGregor; Connor Wickham; Brandon Pierrick; ",
            null,null,null,"2020-01-18",null,"18/01/20",
            "15:00:00","16:00:00",null,"133613","133632",
            null,null,null,null,null,null,null,null,
            null,null,null,null,null,"unlocked")

        // create empty leagueDataList
        val leagueDataList: MutableList<League> = mutableListOf()
        // add a league to leagueDataList
        leagueDataList.add(league)
        // put leagueDataList into leagueResponse
        val leagueResponse = LeagueResponse(leagueDataList)

        // create empty nextEventList
        val nextEventList: MutableList<Event> = mutableListOf()
        // add an event to nextEventList
        nextEventList.add(nextEvent)
        // put nextEventList into nextEventResponse
        val nextEventResponse = EventResponse(nextEventList)
        // create empty list for next matches of all leagues
        val allLeaguesNextMatchDataList: ArrayList<EventResponse> = ArrayList()
        // add a list of next matches in form of EventResponse object to allLeaguesNextMatchDataList
        allLeaguesNextMatchDataList.add(nextEventResponse)

        // create empty prevEventList
        val prevEventList: MutableList<Event> = mutableListOf()
        // add an event to prevEventList
        prevEventList.add(prevEvent)
        // put prevEventList into prevEventResponse
        val previousEventResponse = EventResponse(prevEventList)
        // create empty list for prev matches of all leagues
        val allLeaguesPrevMatchDataList: ArrayList<EventResponse> = ArrayList()
        // add a list of prev matches in form of EventResponse object to allLeaguesPrevMatchDataList
        allLeaguesPrevMatchDataList.add(previousEventResponse)
        ...
        runBlocking {
            ...
            Mockito.`when`(gson.fromJson("", EventResponse::class.java))
                .thenReturn(nextEventResponse).thenReturn(previousEventResponse)

            mainPresenter.getLeagueDataFromAPI(leagueIds)

            Mockito.verify(mainView).showLoading()
            Mockito.verify(mainView).hideLoading()
            Mockito.verify(mainView).showLeagueImageGridList(
                leagueDataList,
